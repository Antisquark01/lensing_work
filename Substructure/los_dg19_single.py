...
class LOSDG19(LOSBase):
	...
	def power_law_dn_dm(self,z,m_min,m_max,n_dm=100):
		...
		# print(slope_estimate, norm_estimate)
	...
	def mass_concentration(self,z,m_200,scatter_mult=1.0):
		...
		# if max(concentrations)>20:
			# print("conc:",z, np.mean(concentrations), max(concentrations))
	...
	def convert_to_lenstronomy(self,z,z_masses,z_cart_pos):
		...
		concs = np.load("data/c2.npy")
		concs = np.concatenate([concs, concentration])
		np.save("data/c2",concs)
	...
	def draw_los(self):
		...
		z_range = np.arange(0.2,2.5,dz)
		# z_range = np.arange(0.1,2.0,dz)
		# z_range = np.arange(0.1,6,dz)

		# z_range = np.arange(.45,.55,dz)
		...
		for z in z_range:
			...
			# z_masses = np.zeros(len(z_masses)*10000)
			all_masses += list(z_masses)
			# z_masses = np.array([1e11]*5)
		...
		# print("masses:",np.array(all_masses)/1e11)
		print(len(los_z_list))

		# theta_Es = [opt.root(lambda x: LensModel(['NFW'], z_lens=los_z_list[i], z_source=z_source).ray_shooting(x,0, [{'alpha_Rs': los_kwargs_list[i]['alpha_Rs'], 'Rs': los_kwargs_list[i]['Rs'], 'center_x': 0., 'center_y': 0.}])[0], 5).x[0] for i in range(len(los_z_list))]
		# theta_Es = np.array(theta_Es)
		# inds = (theta_Es>=0.8) * (theta_Es<=1.5)
		# print("theta_E:",theta_Es[inds])
		all_masses = np.array(all_masses)
		print(np.sort(np.array(all_masses))/1e12)
		# print(len(theta_Es[inds]), len(los_z_list))

		print(len(all_masses[all_masses>1e11]), len(all_masses[all_masses>5e11]), len(all_masses[all_masses>1e12]))
		# print(len(all_masses[all_masses>5e9]), len(all_masses[all_masses>5e10]), len(all_masses[all_masses>5e11]), len(all_masses[all_masses>5e12]))
		# print([los_kwargs_list[i]['center_x'] for i in range(len(los_kwargs_list))])
		# r = np.random.random()
		# theta = np.random.random()*2*np.pi
		# los_kwargs_list[0]['center_x'] = r*np.cos(theta)
		# los_kwargs_list[0]['center_y'] = r*np.sin(theta)
		# print(los_z_list)
		np.save("data/halo_masses2", all_masses)
		# np.save("data/halo_thetaE2", theta_Es)
		np.save("data/halo_zs2", np.array(los_z_list))
		np.save("data/halo_params2", np.array(los_kwargs_list))
